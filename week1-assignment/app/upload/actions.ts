...
export async function registerAndGenerateCaptions(cdnUrl: string) {
  ...
  if (!registerResponse.ok) {
    ...
    throw new Error(`Failed to register image: ${registerResponse.status} ${errorText}`)
  }

  const { imageId } = await registerResponse.json()

  // Step 4: Generate Captions
  const generateResponse = await fetch(`${API_BASE}/pipeline/generate-captions`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
  ...
  if (!generateResponse.ok) {
    ...
    throw new Error(`Failed to generate captions: ${generateResponse.status} ${errorText}`)
  }
